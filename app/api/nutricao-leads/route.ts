...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const resp = await client.messages.create({
      ...
      messages: [{
        role:    "user",
        content: `Você é especialista em nutrição de leads médicos para o médico usuário.
...
Retorne APENAS o JSON array, sem markdown.`,
    ...
    const dias2= parseAIJson(idx >= 0 ? clean.slice(idx) : clean)
    return NextResponse.json({ trilha: dias2 })
  } catch (e) {
    captureAnthropicError(e, "/api/nutricao-leads")
