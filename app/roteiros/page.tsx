...
import { Copy, Check, Loader2, Sparkles, ClipboardList, Clapperboard, Zap, FileText, Hash, X, Mic, Video, Scissors } from "lucide-react"
...
function CopyBtn({text,label='Copiar'}:{text:string;label?:string}){const[ok,setOk]=useState(false);return<button onClick={()=>{navigator.clipboard.writeText(text);setOk(true);setTimeout(()=>setOk(false),1600)}} style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:6,border:`1px solid ${ok?D.aborder:D.border}`,background:ok?D.adim:'none',color:ok?D.atext:D.muted,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:D.font}}>{ok?<Check className="w-3.5 h-3.5" />:<Copy className="w-3.5 h-3.5" />} {ok?'Copiado':label}</button>}
    ...
    <div className="min-h-screen" style={{fontFamily:D.font,color:D.text}}>
      ...
      <div style={{display:'flex',height:isMob?'auto':'calc(100vh - 68px)',flexDirection:isMob?'column':'row'}}>
        ...
        <div className="bg-background" style={{flex:1,overflowY:'auto',padding:isMob?14:24}}>
          {!roteiro&&!loading&&<div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'55vh',gap:10,opacity:.35}}><Clapperboard style={{width:44,height:44}}/><div style={{color:D.muted,fontSize:11,fontWeight:700,letterSpacing:3,textTransform:'uppercase',textAlign:'center',fontFamily:D.mono}}>Defina o tema e clique<br/>em Gerar Roteiro</div></div>}
          ...
          {roteiro&&!loading&&<>
            <div style={{marginBottom:18}}>
              ...
              <div style={{padding:'10px 14px',background:D.adim,border:`1px solid ${D.aborder}`,borderRadius:8,display:'flex',gap:10,alignItems:'flex-start',marginBottom:14}}>
                <Zap style={{width:16,height:16,color:D.atext,flexShrink:0}}/><div><div style={{color:D.atext,fontSize:9,fontWeight:700,letterSpacing:3,textTransform:'uppercase',marginBottom:3,fontFamily:D.mono}}>GANCHO</div><div style={{color:D.text,fontSize:13,fontStyle:'italic',lineHeight:1.5}}>"{roteiro.hook}"</div></div>
              </div>
              <div style={{display:'flex',gap:2,borderBottom:`1px solid ${D.border}`}}>
                {(['roteiro','legenda','hashtags'] as const).map(tab=><button key={tab} onClick={()=>setActiveTab(tab)} style={{padding:'7px 14px',border:'none',borderBottom:activeTab===tab?`2px solid ${D.accent}`:'2px solid transparent',background:'none',color:activeTab===tab?D.accent:D.muted,fontWeight:700,fontSize:10,cursor:'pointer',textTransform:'uppercase',letterSpacing:2,fontFamily:D.mono,marginBottom:-1}}>{tab==='roteiro'?<><Clapperboard style={{width:12,height:12,verticalAlign:'-0.15em',marginRight:4}}/>Roteiro</>:tab==='legenda'?<><FileText style={{width:12,height:12,verticalAlign:'-0.15em',marginRight:4}}/>Legenda</>:<><Hash style={{width:12,height:12,verticalAlign:'-0.15em',marginRight:4}}/>Hashtags</>}</button>)}
              ...
              {roteiro.blocos.map((b,idx)=>{const cfg=TIPO[b.tipo];const isEd=editIdx===idx;return(
                <div key={b.id} style={{background:D.card,border:`1px solid ${isEd?D.aborder:D.border}`,borderRadius:9,overflow:'hidden',borderLeft:`3px solid ${cfg.color}`}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,padding:'9px 13px',borderBottom:`1px solid ${D.border}`,background:D.surface}}>
                    ...
                    <button onClick={()=>{setEditIdx(isEd?null:idx);setEditInstr('')}} style={{marginLeft:'auto',padding:'3px 9px',borderRadius:5,border:`1px solid ${isEd?D.aborder:D.border}`,background:isEd?D.adim:'none',color:isEd?D.atext:D.muted,fontSize:10,fontWeight:700,cursor:'pointer',fontFamily:D.font}}>{isEd?<X style={{width:12,height:12}}/>:<><Sparkles style={{width:12,height:12,verticalAlign:'-0.15em',marginRight:4}}/>Editar</>}</button>
                  </div>
                  <div style={{padding:'12px 14px',display:'flex',flexDirection:'column',gap:9}}>
                    <div><div style={{fontSize:9,fontWeight:700,color:D.muted,letterSpacing:3,textTransform:'uppercase',marginBottom:4,fontFamily:D.mono}}><Mic style={{width:11,height:11,verticalAlign:'-0.15em',marginRight:4}}/>FALA</div>{isEd?<textarea value={b.fala} rows={3} onChange={e=>setRoteiro(prev=>prev?{...prev,blocos:prev.blocos.map((bl,i)=>i===idx?{...bl,fala:e.target.value}:bl)}:prev)} style={{...inp,resize:'none',display:'block',fontSize:13}}/>:<div style={{color:D.text,fontSize:13,lineHeight:1.6}}>{b.fala}</div>}</div>
                    <div style={{display:'grid',gridTemplateColumns:isMob?'1fr':'1fr auto',gap:9}}>
                      <div><div style={{fontSize:9,fontWeight:700,color:D.muted,letterSpacing:3,textTransform:'uppercase',marginBottom:4,fontFamily:D.mono}}><Video style={{width:11,height:11,verticalAlign:'-0.15em',marginRight:4}}/>VISUAL</div>{isEd?<input value={b.visual} onChange={e=>setRoteiro(prev=>prev?{...prev,blocos:prev.blocos.map((bl,i)=>i===idx?{...bl,visual:e.target.value}:bl)}:prev)} style={{...inp,fontSize:12}}/>:<div style={{color:D.text2,fontSize:12,lineHeight:1.5}}>{b.visual}</div>}</div>
                      <div style={{minWidth:130}}><div style={{fontSize:9,fontWeight:700,color:D.muted,letterSpacing:3,textTransform:'uppercase',marginBottom:4,fontFamily:D.mono}}><Scissors style={{width:11,height:11,verticalAlign:'-0.15em',marginRight:4}}/>CORTE</div>{isEd?<input value={b.corte} onChange={e=>setRoteiro(prev=>prev?{...prev,blocos:prev.blocos.map((bl,i)=>i===idx?{...bl,corte:e.target.value}:bl)}:prev)} style={{...inp,fontSize:11}}/>:<span style={{display:'inline-block',padding:'3px 9px',borderRadius:20,background:D.adim,border:`1px solid ${D.aborder}`,color:D.atext,fontSize:10,fontWeight:700,fontFamily:D.mono}}>{b.corte}</span>}</div>
                    </div>
                    {isEd&&<div style={{paddingTop:9,borderTop:`1px solid ${D.border}`}}><div style={{fontSize:9,fontWeight:700,color:D.atext,letterSpacing:3,textTransform:'uppercase',marginBottom:7,fontFamily:D.mono}}><Sparkles style={{width:11,height:11,verticalAlign:'-0.15em',marginRight:4}}/>REGENERAR COM IA</div><div style={{display:'flex',gap:7}}><input value={editInstr} onChange={e=>setEditInstr(e.target.value)} placeholder="Ex: mais impactante, use dado do NEJM..." style={{...inp,flex:1}} onKeyDown={e=>{if(e.key==='Enter'&&editInstr.trim())regenBloco(idx)}}/><button onClick={()=>regenBloco(idx)} disabled={regenLoad||!editInstr.trim()} style={{padding:'9px 12px',borderRadius:7,border:`1px solid ${D.aborder}`,background:editInstr.trim()?D.adim:'none',color:editInstr.trim()?D.atext:D.muted,fontWeight:700,fontSize:11,cursor:editInstr.trim()?'pointer':'not-allowed',fontFamily:D.font}}>{regenLoad?<Loader2 className="animate-spin" style={{width:14,height:14}}/>:<Zap style={{width:14,height:14}}/>}</button></div></div>}
                  </div>
                </div>
              )})}
            </div>}
            {activeTab==='legenda'&&<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:9}}><span style={{fontSize:9,fontWeight:700,color:D.muted,letterSpacing:3,textTransform:'uppercase',fontFamily:D.mono}}>{roteiro.legenda.length}/2200</span><CopyBtn text={roteiro.legenda+'\n\n'+roteiro.hashtags.map(h=>'#'+h.replace(/^#/,'')).join(' ')} label="Copiar legenda"/></div><textarea value={roteiro.legenda} rows={16} onChange={e=>setRoteiro(prev=>prev?{...prev,legenda:e.target.value}:prev)} style={{...inp,resize:'none',display:'block',lineHeight:1.7,fontSize:13}}/></div>}
            {activeTab==='hashtags'&&<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><span style={{fontSize:9,fontWeight:700,color:D.muted,letterSpacing:3,textTransform:'uppercase',fontFamily:D.mono}}>{roteiro.hashtags.length} hashtags</span><CopyBtn text={roteiro.hashtags.map(h=>'#'+h.replace(/^#/,'')).join(' ')} label="Copiar todas"/></div><div style={{display:'flex',flexWrap:'wrap',gap:7}}>{roteiro.hashtags.map((h,i)=><div key={i} style={{display:'flex',alignItems:'center',background:D.card,border:`1px solid ${D.border}`,borderRadius:20,overflow:'hidden'}}><span style={{padding:'6px 11px',color:D.atext,fontSize:12,fontWeight:700,fontFamily:D.mono}}>#{h.replace(/^#/,'')}</span><button onClick={()=>setRoteiro(prev=>prev?{...prev,hashtags:prev.hashtags.filter((_,j)=>j!==i)}:prev)} style={{padding:'6px 9px',background:'none',border:'none',color:D.muted,cursor:'pointer',fontSize:10,display:'flex'}}><X style={{width:12,height:12}}/></button></div>)}</div><div style={{marginTop:14,display:'flex',gap:7}}><input id="ht" type="text" placeholder="Adicionar hashtag..." style={{...inp,flex:1}} onKeyDown={e=>{if(e.key==='Enter'){const v=(e.target as HTMLInputElement).value.replace('#','').trim();if(v){setRoteiro(prev=>prev?{...prev,hashtags:[...prev.hashtags,v]}:prev);(e.target as HTMLInputElement).value=''}}}}/><button onClick={()=>{const i=document.getElementById('ht') as HTMLInputElement;const v=i.value.replace('#','').trim();if(v){setRoteiro(prev=>prev?{...prev,hashtags:[...prev.hashtags,v]}:prev);i.value=''}}} style={{padding:'9px 12px',borderRadius:7,border:`1px solid ${D.border}`,background:D.card,color:D.atext,fontWeight:700,fontSize:11,cursor:'pointer',fontFamily:D.font}}>+</button></div></div>}
          </>}
        </div>
      </div>
