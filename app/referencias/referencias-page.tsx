...
export default function ReferenciasPage() {
  ...
  const addRef = async () => {
    ...
    if (error) {
      ...
      showToast("Referência salva com sucesso!")
  ...
  const totalSeguidores = refs
    .map(r => parseInt(r.seguidores.replace(/[^0-9]/g, "")) || 0)
    .reduce((a, b) => a + b, 0)

  const fmtSeguidores = totalSeguidores > 0
    ? totalSeguidores >= 1000000
      ? (totalSeguidores / 1000000).toFixed(1) + "M+"
      : (totalSeguidores / 1000).toFixed(0) + "k+"
    : "—"
    ...
    <div className="animate-fade-in">
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <StatCard label="Total Seguidores"    value={fmtSeguidores}                                sub="alcance combinado"   icon={Eye}        accent="amber" />
        </div>
          ...
          <div className="bg-card border border-accent-border rounded-lg p-6 space-y-4 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <select value={newFreq} onChange={e => setNewFreq(e.target.value)}
                className="bg-background border border-border rounded-lg px-3 py-2 text-[12px] text-text-primary outline-none focus:border-accent/40">
                {["Diária","3x/semana","2x/semana","1x/semana","Esporádica"].map(f => <option key={f}>{f}</option>)}
              </select>
